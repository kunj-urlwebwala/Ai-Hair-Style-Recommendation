...
export const fallbackRecommendations: HairstyleRecommendation[] = [
  {
    ...
    description: "A collarbone-length cut with subtle interior layers and a relaxed center part.",
    whyItWorks: "The soft perimeter keeps the outline polished while face-framing layers add movement.",
    maintenance: "Medium",
  ...
  },
  {
    ...
    description: "Long, loose waves with a wispy curtain fringe that opens softly at the brow.",
    whyItWorks: "The open fringe creates a light frame while the waves bring an easy, romantic finish.",
    maintenance: "Medium",
    texture: "Soft waves",
  ...
  },
  {
    id: "textured-pixie",
    name: "Textured pixie",
    description: "A refined short crop with a longer textured top and gentle side-swept movement.",
    whyItWorks: "The cropped sides focus attention upward while the longer crown leaves room for styling versatility.",
    maintenance: "High",
  ...
  },
  {
    ...
    description: "A chin-skimming, softly rounded bob with a deep side part and glossy finish.",
    whyItWorks: "The precise outline feels intentional, while the soft bend at the ends prevents the look from becoming severe.",
    maintenance: "Medium",
...
function normalizeRecommendation(value: unknown, index: number): HairstyleRecommendation {
  const fallback = fallbackRecommendations[index % fallbackRecommendations.length];
  const raw = typeof value === "object" && value !== null ? (value as Record<string, unknown>) : {};
  const safeId = asText(raw.id, fallback.id).toLowerCase().replace(/[^a-z0-9-]+/g, "-");
  return {
...
export function parseStyleAnalysis(content: string): Omit<ConsultationResponse, "sourceImageUrl"> {
  ...
  return {
    ...
    analysis: {
      ...
      featureNotes: asStringArray(analysisCandidate.featureNotes, fallbackStyleAnalysis.featureNotes),
      stylePrinciples: asStringArray(analysisCandidate.stylePrinciples, fallbackStyleAnalysis.stylePrinciples),
      confidenceNote: asText(analysisCandidate.confidenceNote, fallbackStyleAnalysis.confidenceNote).slice(0, 220),
    },
    recommendations: recommendationsCandidate.length >= 3 ? recommendationsCandidate.slice(0, 4).map(normalizeRecommendation) : fallbackRecommendations,
  };
}
